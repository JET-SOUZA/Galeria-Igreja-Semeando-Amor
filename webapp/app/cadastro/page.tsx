...
export default function Cadastro(){
 ...
 return <main className="register-page" style={{backgroundColor:branding.background_color||undefined,color:branding.text_color||undefined}}><div className="register-shell" style={{borderRadius:branding.card_radius||undefined}}><section className="register-brand" style={branding.cover_url?{backgroundImage:`linear-gradient(160deg,#000b,#0007),url(${branding.cover_url})`,backgroundSize:'cover',backgroundPosition:'center'}:{}}><a className="brand-lockup" href="/"><img src={logo} alt="Logo"/><div><strong>{name}</strong><span>{display}</span></div></a><div className="register-copy"><span className="hero-kicker">Acesso às galerias</span><h1>{branding.welcome_title||<>Antes de entrar,<br/><em>queremos conhecer você.</em></>}</h1><p>{branding.welcome_subtitle||`Seu cadastro libera o acesso às fotos e recursos disponibilizados por ${display}.`}</p><div className="register-benefits"><span>✓ Acesso às galerias autorizadas</span><span>✓ Busca facial quando disponível</span><span>✓ Cadastro feito uma única vez neste aparelho</span></div></div>{branding.show_developer_signature!==false&&<div className="register-developer"><span>Tecnologia e desenvolvimento</span><img src={DEVELOPER_LOGO} alt={DEVELOPER_NAME}/><strong>{DEVELOPER_NAME}</strong></div>}</section>
 <form className="register-card" onSubmit={mode==='recover'?submitRecovery:submitRegistration} style={{borderRadius:branding.card_radius||undefined}}><div className={styles.tabs} role="group" aria-label="Escolha como acessar"><button type="button" className={mode==='recover'?styles.active:''} aria-pressed={mode==='recover'} onClick={()=>changeMode('recover')}>Já tenho cadastro</button><button type="button" className={mode==='register'?styles.active:''} aria-pressed={mode==='register'} onClick={()=>changeMode('register')}>Criar cadastro</button></div><div><span className="eyebrow">{mode==='recover'?'Acesso':'Cadastro'}</span><h2>{mode==='recover'?'Recuperar meu acesso':branding.registration_title||'Libere seu acesso'}</h2><p className="muted">{mode==='recover'?'Confirme seus dados para voltar às fotos.':branding.registration_subtitle||'Preencha os dados abaixo.'}</p></div>{error&&<p className="notice" role="alert">{error}</p>}{mode==='recover'?<div className={styles.recoveryFields}><label>CPF<input required inputMode="numeric" autoComplete="off" value={recovery.cpf} onChange={event=>setRecoveryValue('cpf',formatCpf(event.target.value))}/></label><label>Data de nascimento<input required type="date" autoComplete="bday" value={recovery.birth_date} onChange={event=>setRecoveryValue('birth_date',event.target.value)}/></label><label>WhatsApp<input required inputMode="tel" autoComplete="tel" value={recovery.whatsapp} onChange={event=>setRecoveryValue('whatsapp',formatPhone(event.target.value))}/></label></div>:groups.map((group:any[],index:number)=>group.length===2?<div className="form-row" key={index}>{group.map(renderField)}</div>:renderField(group[0]))}<button className="btn register-submit" disabled={busy} style={{borderRadius:branding.button_radius||undefined}}>{busy?'Validando...':mode==='recover'?'Entrar e voltar às fotos':'Continuar para as fotos'}</button><p className="form-footnote">{mode==='recover'?'Usamos estes dados somente para localizar seu cadastro neste evento.':'Seus dados não são públicos. Campos opcionais podem ser deixados em branco.'}</p></form></div></main>;
}
