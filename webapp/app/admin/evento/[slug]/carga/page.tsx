...
export default function MassUpload({params}:{params:{slug:string}}){
  ...
  return <main className="mass">
    ...
    <section className="picker"><div><span>01 • SELECIONAR FOTOS</span><h2>Monte a fila em partes</h2><p>A fila aceita milhares de fotos. No iPhone, adicione até {SAFE_PICK_SIZE} por seleção e repita: todas ficam reunidas na mesma fila para um único envio. No computador, você também pode escolher uma pasta inteira.</p><p className="iphoneNote"><b>Original preservado:</b> para conservar o HEIC exato, salve/exporte no app Arquivos e selecione por lá. A Fototeca pode entregar uma cópia JPEG ao navegador.</p></div><div className="pickButtons"><label>＋ Adicionar fotos<input hidden type="file" multiple accept="image/*,.heic,.heif,.tif,.tiff,.cr2,.cr3,.nef,.arw,.dng,.raf,.orf,.rw2,.pef" onChange={pick}/></label><label className="folder">▣ Adicionar pasta<input hidden type="file" multiple accept="image/*,.heic,.heif,.tif,.tiff,.cr2,.cr3,.nef,.arw,.dng,.raf,.orf,.rw2,.pef" {...({webkitdirectory:'',directory:''} as any)} onChange={pick}/></label>{files.length>0&&!running&&<button className="clear" onClick={()=>{setFiles([]);setFailures([]);setRejected([]);setIgnoredFiles([]);setMsg('Fila local limpa. Nenhuma foto já enviada foi apagada.')}}>Limpar seleção local</button>}</div></section>
