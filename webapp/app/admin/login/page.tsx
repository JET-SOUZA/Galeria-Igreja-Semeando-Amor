...
export default function Login(){
 ...
 useEffect(()=>{const s=readSession();if(s?.access_token)routeByRole(s.access_token).then(ok=>{if(!ok){localStorage.removeItem('semeando_admin_session');sessionStorage.removeItem('semeando_admin_session')}})},[]);
 async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{
  ...
  const ok=await routeByRole(d.access_token);if(!ok)throw new Error('Esta conta não possui acesso administrativo ativo.');
