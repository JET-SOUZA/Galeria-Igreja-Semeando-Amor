...
export async function POST(req:NextRequest){
 ...
 try{
  const body=await req.json();jobId=String(body.job_id||'');workerToken=String(body.worker_token||'');const link=String(body.link||'').trim();if(!jobId||!workerToken||!link)return NextResponse.json({error:'job_id, worker_token e link são obrigatórios.'},{status:400});
  const target=normalize(link),candidates=new Map<string,Candidate>();
  ...
  // Also include rendered CDN image URLs because some responses omit content-length.
  ...
  if(!candidates.size)throw new Error('O iCloud abriu, mas nenhuma imagem do compartilhamento foi descoberta. O link pode ter expirado, exigir interação adicional ou a Apple pode ter alterado o visualizador.');
  const all=[...candidates.values()].sort((a,b)=>b.size-a.size);
