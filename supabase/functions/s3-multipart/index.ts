...
const B2_AK=(Deno.env.get('B2_APPLICATION_KEY_ID')||'').trim();
const B2_SK=(Deno.env.get('B2_APPLICATION_KEY')||'').trim();
...
async function sessionScope(c:any,id:string){
  ...
  if(s.provider!=='s3')throw Error('SESSION_PROVIDER_INVALID');
  return s;
}

function s3(row:any){
  if(!B2_AK||!B2_SK)throw Error('B2_SECRETS_MISSING');
  if(!row?.endpoint||!row?.bucket_name||!row?.region)throw Error('S3_NOT_CONFIGURED');
  if(row?.config?.vendor!=='backblaze_b2')throw Error('S3_VENDOR_UNSUPPORTED');
  return new S3Client({
    region:String(row.region),
    endpoint:String(row.endpoint),
    forcePathStyle:true,
    credentials:{accessKeyId:B2_AK,secretAccessKey:B2_SK}
  });
}
...
async function backendFor(c:any,backendId:string){
  ...
  if(!b)throw Error('S3_NOT_CONFIGURED');
  return b;
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(action==='init'){
      ...
      if(!space||!backend||backend.provider!=='s3'||backend.config?.vendor!=='backblaze_b2'||!backend.active||backend.readiness_status!=='ready')return j({error:'Este evento ainda não está apontando para um backend S3/B2 ativo.',code:'S3_EVENT_NOT_READY'},409);
    ...
    const code=m==='UNAUTHORIZED'?401:m==='FORBIDDEN'?403:['EVENT_NOT_FOUND','SESSION_NOT_FOUND'].includes(m)?404:['B2_SECRETS_MISSING','S3_NOT_CONFIGURED','S3_VENDOR_UNSUPPORTED','SESSION_PROVIDER_INVALID'].includes(m)?409:500;
